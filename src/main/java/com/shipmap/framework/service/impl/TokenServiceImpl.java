...
import com.shipmap.modules.system.dao.*;
...
public class TokenServiceImpl implements TokenService {
    ...
    private Map<String, NavigationBarVo> getMenus(List<Authority> authorityList) {
        ...
        for (int i = 0; i < menuList.size(); i++) {
            ...
            List<MenuVo> temp_menuList = menuGroupMap.get(authority.getParentId());
            MenuVo menuVo = new MenuVo();
            menuVo.setName(authority.getAuthorityName());
            menuVo.setUrl(authority.getMenuUrl());
            menuVo.setIcon(authority.getMenuIcon());
            if (temp_menuList == null) {
                temp_menuList = new ArrayList<MenuVo>();
                temp_menuList.add(menuVo);
                menuGroupMap.put(authority.getParentId(), temp_menuList);
            } else {
                temp_menuList.add(menuVo);
            }
        }
    ...
    private String[] getButtonAuthority(List<Authority> list) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (authority.getType() == 3) {
                //只记录按钮权限
                temp_list.add(authority.getAuthority());
            }
        }
        String[] arr = new String[temp_list.size()];
        temp_list.toArray(arr);
        return arr;
    }
    ...
    public boolean addAuth(User user, String... roleIds) throws UserExistException, RoleNotFoundException {
        User temp_user = userMapper.getByUsername(user.getUsername());
        if (temp_user != null) {
    ...
    public boolean updateAuth(User user, String... roleIds) throws UserExistException, RoleNotFoundException {
        User temp_user = userMapper.selectById(user.getId());
        if (temp_user == null) {
            //用户不存在
            throw new UserNotFoundException();
        }
        ...
        temp_user.setNickName(user.getNickName());
        temp_user.setSex(user.getSex());
        temp_user.setRoles(roles);
        temp_user.setPhone(user.getPhone());
        Integer number = userMapper.updateById(temp_user);
        for (int i = 0; i < roles.size(); i++) {
            //用户角色
            UserRole userRole = new UserRole();
            userRole.setUserId(temp_user.getId());
            userRole.setRoleId(roles.get(i).getId());
            userRoleMapper.insert(userRole);
        }
        return toResult(number);
    ...
    public boolean queryAuthIsExistsByUsername(String username) {
        User temp_user = userMapper.getByUsername(username);
        if (temp_user != null) {
            return true;
        }
        return false;
